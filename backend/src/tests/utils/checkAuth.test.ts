...
describe("checkAuthentification", () => {
    it("should throw an error if req.auth is not present", async () => {
        ...
        await expect(checkAuthentification(req)).rejects.toThrow("Unauthorized access")
    })
...
describe("checkUserRole", () => {
    ...
    it("should thorw and error if userRoleIndex is equal to -1", () => {
        ...
        expect(() => checkUserRole(invalideRole)).toThrow("Invalid role")
    })
})
describe("checkUserRole", () => {
    it("should throw an error if userParams is not present", async () => {
        ...
        await expect(checkUserParams(userParamsId)).rejects.toThrow("No conditions met")
